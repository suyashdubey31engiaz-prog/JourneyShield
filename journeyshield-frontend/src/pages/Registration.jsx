...
const Registration = () => {
  ...
  const [wakingUp,    setWakingUp]    = useState(false); // Render cold-start state
          ...
          {[1, 2].map((s) => (
            <React.Fragment key={s}>
              <div className={`flex items-center justify-center w-8 h-8 rounded-full text-xs font-bold border-2 transition-all ${
                step >= s ? 'bg-yellow-500 border-yellow-500 text-black' : 'border-gray-600 text-gray-500'
              }`}>
                {step > s ? '✓' : s}
              </div>
              {s < 2 && <div className={`h-px w-12 transition-all ${step > s ? 'bg-yellow-500' : 'bg-gray-700'}`} />}
            </React.Fragment>
          ))}
        </div>
        ...
        <div className="bg-gray-800/80 backdrop-blur p-8 rounded-2xl border border-gray-700 shadow-2xl">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400 text-sm mt-2">
              {step === 1 ? 'Join Journey Shield and travel safely' : `We sent a 6-digit code to ${formData.email}`}
            </p>
          </div>

          {/* Error */}
          {error && (
            <div className="flex items-start gap-2 bg-red-900/40 border border-red-500/40 text-red-400 p-3 rounded-lg mb-5 text-sm font-medium">
          ...
          {/* Waking up banner */}
          {wakingUp && !error && (
            <div className="flex items-center gap-2 bg-yellow-900/40 border border-yellow-500/40 text-yellow-400 p-3 rounded-lg mb-5 text-sm">
              ...
              Server is waking up — this may take up to 30 seconds on first use...
            </div>
          )}
          ...
          {step === 1 && (
            <>
                ...
                {roles.map((r) => (
                  <button key={r.value} type="button" onClick={() => setRole(r.value)}
                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${
                      role === r.value ? 'bg-yellow-500 text-black shadow-md' : 'text-gray-400 hover:text-white'
                    }`}>
                    {r.icon} {r.label}
                  </button>
                ))}
                ...
                <button type="submit" disabled={loading}
                  ...
                  {loading ? (
                    <>
                      ...
                      {wakingUp ? 'Waking server up...' : 'Sending Code...'}
                    </>
              ...
              <p className="text-gray-400 text-center text-sm mt-6">
                ...
                <Link to="/login" className="text-yellow-400 hover:text-yellow-300 font-semibold hover:underline">Login</Link>
              </p>
            </>
          )}
          ...
          {step === 2 && (
            <form onSubmit={handleVerifyAndRegister} className="space-y-5 animate-fadeIn">
              ...
              <input type="text" maxLength="6" inputMode="numeric" pattern="[0-9]*"
                placeholder="0  0  0  0  0  0" required value={otp}
...
//I dont want to be exceptional, i dont want to make history, i dont want to be a legend, i just want to be one of Many.